...
import { sceneScript } from "./sceneScript_coffee";
import { sceneManager } from "../../../../components/SceneManager/SceneManager";
import { Dialog } from "../../../../components/Dialog";

export class CoffeShopOnboarding extends DefaultScene {
  dialogManager;

  constructor() {
    super({ key: "CoffeShopOnboarding" });
    this.sceneManager = sceneManager;
    this.dialogManager = new Dialog ('onboarding');
  }

  create() {
    const image = this.add.image(0, 0, "coffeShop").setOrigin(0, 0);
    ...
    const jumpScene = () => {
      if (!handled) {
        this.scene.stop("CoffeShopOnboarding");
        this.scene.launch("TheOfficeOnboarding");
        handled = true;
      }
    }
    ...
    this.events.once("postupdate",() => {
        setTimeout(async () => {
          ...
          sceneManager.status["explanationCafeteria"] = true;
